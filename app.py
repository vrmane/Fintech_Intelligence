...
with c_h1:
    if not pos_bases.empty and theme_cols:
        ...
        st.plotly_chart(px.imshow(p_pct, text_auto='.1f', aspect="auto", color_continuous_scale='Greens', title=f"Top Drivers (% of Positive) | Base: {pos_bases.sum():,}"), use_container_width=True, key="db_pos")
...
with c_h2:
    if not neg_bases.empty and theme_cols:
        ...
        st.plotly_chart(px.imshow(n_pct, text_auto='.1f', aspect="auto", color_continuous_scale='Reds', title=f"Top Barriers (% of Negative) | Base: {neg_bases.sum():,}"), use_container_width=True, key="db_neg")
