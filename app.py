...
st.markdown("""
    <style>
        ...
        hr { border-color: #334155; margin: 2em 0; }
    </style>
...
with tab_exec:
    ...
    # Calculate MoM Deltas per Brand (Tricky but valuable)
    deltas = []
    ...
    # Display as a styled dataframe for quick scanning
    st.dataframe(
        final_kpi.style.background_gradient(subset=['CSAT'], cmap='Greens').background_gradient(subset=['NPS Proxy'], cmap='RdYlGn'),
    ...
    with c1:
        st.markdown("#### 🏆 Market Share (Volume)")
        fig_don = px.pie(kpi_df, values='Vol', names='App_Name', hole=0.4, 
                         title="Volume Share by Brand", hover_data=['Vol'])
    ...
    with c2:
        ...
        fig_scat = px.scatter(kpi_df, x="CSAT", y="NPS Proxy", size="Vol", color="App_Name", 
                              text="App_Name", title="Strategic Positioning", 
                              hover_data={'Vol':True, 'CSAT':':.2f', 'NPS Proxy':':.0f'})
